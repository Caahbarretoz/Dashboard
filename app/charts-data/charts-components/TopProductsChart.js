...
const TopProductsChart = () => {
  return (
    <div className="relative flex flex-col items-center w-[98%] h-full text-titles">
        ...
        <table className="w-full table-auto border-collapse text-[10px]">
          ...
          {Data.data.datasets.map((dataset, index) => (
            <tbody key={index}>
              <tr>
                <td
                  className={`px-4 ${dataset.customPadding} border-t border-gray-200 text-[#999]`}
                >
                  {dataset.index}
                </td>
                <td
                  className={`px-4 ${dataset.customPadding} border-t border-gray-200 uppercase text-[#999]`}
                >
                  {dataset.product}
                </td>
                <td
